...
- [1] 重构敌人ai 🔼
	...
	- [1] 回血
	- [ ] 进行其他的操作
- [2] 敌人采取的策略类型（策略类型目前只在移动后攻击上有区别） ❌ 2026-02-27
	- [2] 激进型——会一直跟着玩家 ❌ 2026-02-27
	- [2] 防御型——根据玩家上一回合的位置进行攻击 ❌ 2026-02-27
	- [2] 策略型——会尽量和玩家保持最大距离去攻击 ❌ 2026-02-27
...
![[assets/card game(TODO)/file-20250304164009585.png]]

![[assets/card game(TODO)/file-20250304164103803.png]]
## 参考图
![[assets/card game(TODO)/file-20250303134548923.png]]
![[assets/card game(TODO)/file-20250303134600785.png]]
- [ ] 添加更多地形效果
...
- [ ] 破坏/建造地形系统 ⏫ ➕ 2025-02-12 🛫 2025-02-12 ✅ 2025-02-16 ^dixing
	- [x] 使用地形类型卡牌时将进入建造/破坏模式。 ✅ 2025-02-23
		...
		- [1] 破坏地形的牌的规则是，在第一层不能破坏对应的地砖，但是可以破坏掉对应的地形(effect)，所有地形牌基本都有**消耗**效果。
		- [ ] 
	- [ ] 建造/破坏模式下可以选择对应地砖
 ...
 - [2] 或许重构最基础的Grid系统，改用TileMap，先初步迭代出原型 ➕ 2025-02-12 ❌ 2025-02-12——SBUnity-Tilemap根本满足不了我强烈的增删需求。 ❌ 2025-04-29
...
- [1] 添加地块随机事件系统。
	...
	- [ ] 随机地块的OBJ位于cell的子物体
	- [x] 🛫 2025-04-26 现在的问题是在携带randomEvent的tile检索解锁条件的时候因为这个时候还没有玩家生成，所以会产生报空错误，我一开始的解决方案是将玩家用事件侦听的方式传递进去，但是在顺序上不可能，因为地形会先生成。 ✅ 2025-04-29
``` ad-note
	如果检索解锁条件的时机从生成时改为玩家生成并结束回合后，是不是就能修复这个BUG？
```

- [x] 增加完整度，添加休息房，商店房和宝箱房还有随机房 ✅ 2025-11-19
	- [ ] 休息房
	- [ ] 商店房
	- [ ] 宝箱房
	- [ ] 随机房
...
- [ ] 
- [x] 🛫 2025-04-27  在回合结束敌人移动时，将摄像机主要放置到敌人身上，让玩家能够关注到每一个敌人的行动(因为不止一层) ✅ 2025-05-22
	- [x] 如何移动摄像机->当敌人移动时将摄像机移动到当前层 ✅ 2025-05-22
- [1] 库库搭框架~🛫 2025-04-29  重构一下CardEffect和scriptableEffect让卡牌的伤害以及等等数据能够实现在游戏运行时的动态修改(俗称buff)，目前这一buff系统的设计只涵盖到玩家的状态如血条，移动范围等，并不支持对于卡牌中一部分数值的修改(如伤害，作用范围、回合)等等。
	- [1] 添加更多的buff类型
- [1] 优化数值设计，向桌游的低数值设计看齐看齐
- [ ] 添加位置检测系统，部分卡牌在玩家和敌人位置和面朝方向不同时有特殊效果
- [x] 完善buff系统，在重复添加同种buff的时候叠加回合数。 ✅ 2025-05-15
- [2] 把地块从8x8改成6x6 ❌ 2025-05-18
- [ ] 添加shop房间和更多卡牌
- [ ] 将移动功能从按钮重构到通过卡牌实现
- [x] 重构GameEvent，使其能够支持显示事件的抛出者——以便后续代码的维护 ➕ 2025-05-21 ✅ 2025-05-21
- [x] 敌人防御重置 ✅ 2025-05-23
- [ ] 添加能力类的卡牌，作用于整场战斗的效果(Ability)
- [x] 重构敌人脚本，使用策略模式的方式，在每个子策略的内部进行实际的计算->改用接口，将敌人的移动，攻击等方式用接口实现，具体的参数在策略中 🛫 2025-07-13 ⏳ 2025-07-14 📅 2025-07-14 ✅ 2026-06-17
	- [x] 修复逻辑上的错误，当敌人移动时遇上玩家，发生碰撞而停下脚步 ✅ 2026-06-17
- [x] 丰富敌人种类，添加更多种敌人 🛫 2025-07-13 ⏳ 2025-07-14 📅 2025-07-14 ✅ 2026-06-17
- [x] 添加关卡的configSO文件，以生成对应config下一定范围内浮动的敌人种类以及敌人数量。 ✅ 2025-11-19
- [x] 优化血条设计，把实体的血条改为受击次数，护甲改为临时的受击次数 ✅ 2025-06-12
- [2] 修改一下现在地块的层级逻辑，将overlayTile的逻辑计算部分与表现层分开 ❌ 2025-06-24
- [ ] 添加地形耐久度系统，删除破坏类型的卡牌
- [ ] ![[assets/card game(TODO)/任务线.png]]
- [ ] 说明面板(更加细致的说明，类似陷阵之志中的说明面板，技术层面上讲ITB应该是做了对应的GIF，然后制作了对应的预制体) 🛫 2026-06-17 📅 🔼 ⏳ 2026-06-23 
![[assets/card game(TODO)/file-20260617211559603.png]]
![[assets/card game(TODO)/file-20260617211727482.png]]

- [x] 添加关卡内玩家生成区域和敌人生成区域 🛫 2025-09-25 📅 2025-09-28 ✅ 2025-11-19
	- [x] 关卡模板内规定敌人生成与玩家生成区域固定 ✅ 2025-11-19
	- [x] 出生区域参考： ✅ 2025-11-19
		- [ ] ![[assets/card game(TODO)/file-20250925224908071.png]]
	- [ ] 开始部署时UI显示参考：
		- [ ] ![[assets/card game(TODO)/file-20250925222835073.png]]
		- [ ] ![[assets/card game(TODO)/file-20250925222845319.png]]
- [1] 添加上下层与本层的视觉效果
	- [ ] 上下层虚化
	- [x] 本层添加选中效果，类似于下图： ✅ 2026-01-04
		- [x] ![[assets/card game(TODO)/选中当前层.png]] ✅ 2026-01-04  🏁 ![[assets/card game(TODO)/楼层选中.gif]]
- [ ] 完善敌人info面板，支持鼠标悬停&详细状态面板 🔺 ➕ 2025-11-19
	- [x] 随之伴随而来的是我们的战斗房间中的鼠标悬停逻辑修改：鼠标左键选中敌人，之后可以右键取消选中。 ✅ 2025-11-28
	- [x] 其他的锁定逻辑显示也同陷阵之志一样 ✅ 2026-01-04
		- [x] 选中敌人之后的红色描边![[assets/card game(TODO)/选中敌人之后的表现.png]] 🛫 2025-11-28 ✅ 2026-01-04
		- [x] 选中友军之后的绿色描边 ![[assets/card game(TODO)/选中友军之后的表现.png]] 🛫 2025-11-28 ✅ 2026-01-04
	- [ ] 优化寻路表现 🛫 2025-11-28
		- [ ] 优化玩家寻路时没有实时更新对应的移动路径(没有清除原先选择的路径下的arrowSprite) ![[assets/card game(TODO)/2025.11.28 移动下显示bug.png]]
		- [ ] 优化移动下的显示逻辑，也支持预览效果，详细效果如图：![[assets/card game(TODO)/友军移动时图例.png]] 🛫 2025-11-28
			- [ ] 在原本的地块上显示为一个GhostView(降低alpha值)，在移动到的终点显示玩家的sprite。
			- [ ] 细化对应的overlayTile显示图，不应该用![[assets/card game(TODO)/用于显示移动范围-攻击范围的tile精灵图.png]]这个颜色，移动范围的显示tile图应该都是偏绿色的，详见ITB中的移动范围显示。
	- [ ] 思考多层玩法与我们核心玩法的进一步结合的可能性
		- [ ] 不同层会有不同的buff，比如抽取牌数+1，保留一张牌等等，简称层级buff
		- [ ] 当前层被破坏的地块会产生碎片，对下层(如果有的话)对应地块的敌人造成一定伤害
			- [ ] 碎片机制
				- [ ] 破坏上层地块时，触发下层伤害判定
				- [ ] 伤害范围：下层对应位置3x3区域
				- [ ] 伤害数值：单块5-10，大范围15-20
				- [ ] 碎片下落动画
				- [ ] 下层敌人受伤动画+伤害数字
				- [x] 鼠标悬停预览UI ✅ 2026-04-22
				- [ ] 音效：碎片坠落+敌人受伤
			- [ ] 为了结合上面的这个碎片机制，确保每层都会刷新一个传送点，传送点的机制如下：
				- [x] UI按钮：传送（放在角色面板/快捷栏） ✅ 2026-04-22
				- [ ] 冷却系统：3回合
				- [ ] 消耗判定：本回合移动力清零
				- [ ] 限制判定：本回合未受伤
				- [ ] 目标选择UI：弹出"上层/下层"选项
				...
				- [ ] 状态显示：可用/冷却/不可用
				- [ ] 冷却倒计时显示
	- [ ] 重构玩家的移动力设计，改为行动点，也更好的能够符合传送的设计(不同于固定每回合刷新的移动力，行动点要求玩家合理规划，每回合增加固定点，存在上限)
	- [ ] 填充内容
		- [ ] 卡牌
		- [ ] 敌人设计
			- [ ] 小怪
...
# 优化需求
- [ ] 战斗房间快速移动鼠标时掉帧严重(runtime下1080p稳不住60帧)，需要优化一下性能。
- [x] 扩充敌人美术资产图的padding，让战斗中选中敌人的描边更加清晰 ✅ 2026-06-17
- [ ] 钩子改为十字范围类型的牌。
- [x] 多次叠加虚弱buff失败(UI上显示有问题，不更新层数数字，runtimebuff中是有两层的，但是回合结束后仍然)，多次打出抽牌效果时候后续的牌打出之后没有该效果。敌人被buff弄死之后玩家会进行两个回合(第一个回合只抽牌)，抽两次牌。我们的重置模块目前的buff在回合开始时只会抽取一张牌，而不是卡面描述和配置中的value=2张。 ✅ 2026-06-17
- [ ] 清空单层的敌人之后不会传送到下一层中。


# 设计反思
Q：陷阵之志在战斗层面的设计如下：
![[assets/card game(TODO)/陷阵之志设计.png]]
A：***图中至上而下分别是：***
...
**本人思考:**
由于事先攻击机制的存在，所以玩家躲避攻击变得很容易，这个时候的设计选择要么是像将军对决、方寸对决这种一样降低移动维度(或是从8x8的地块减少到)——这样就能限制玩家的移动能力来让敌人的攻击更有意义;要么是像星之侵略者这样采取别的防御性策略(采用类pvz的思路，玩家需要阻挡敌人下降到最下面的三格，不然就会受到伤害)

既然我的游戏采用了多层8x8的结构，且存在不同效果的地块，那么根据游戏的“合理加减法”，我需要砍掉一部分的功能或者限制玩家的性能来达成这一点，目前的想法是，采取类似于喷射战士中的思路，添加一个染色或者说**摧毁度**的概念，当敌人移动或者攻击时都会产生一定的**摧毁度**(UI上体现在地块颜色的变化)，而玩家的bonus目标就在于这个摧毁度上。或许我该把地块从8x8修改到6x6

另外的另外，我的敌人的攻击上也应该做出一些改变
...
- [1] 游戏UI #Art 
	...
	- [1] 游戏中界面
- [ ] 
- [x] 游戏房间icon ✅ 2025-05-15
- [x] 一些bufficon ✅ 2025-05-15
- [ ] 
## Idea
